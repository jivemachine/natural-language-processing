...
def get_business_text(url):
    url = url
    headers = {'User-Agent': 'inshorts stay informed'}
    response = get(url, headers=headers)
    soup = BeautifulSoup(response.text)
    ...
    cat = re.find(r"inshorts.com/en/read/(\.*)", url)
    blog_dict = [{'title' : title, 'content' : body, 'category' : category}]
    
    return blog_dict

def store_news_locally(blog_dict):
    with open('current_news.txt', 'a') as f:
        f.write(json.dumps(blog_dict))
        

def get_current_news_articles(url):
    blog_dict = get_business_text(url)
    store_news_locally(blog_dict)
    blog_post = open('current_news.txt', 'r')
    return blog_post.read()        
...
# Faith's walkthrough of the functions we wrote
def get_blogs(urls, cache=False):
    '''
